...
export default function HouseholdProfilePage() {
  ...
  const [draft, setDraft] = useState<Partial<HouseholdProfile>>({});
  ...
  async function load() {
    ...
    try {
      setErr(null);
      setLoading(true);
      const res = await fetch(`/api/household/profile?household_id=${encodeURIComponent(householdId)}`, { cache: 'no-store' });
  ...
  async function save() {
    try {
      ...
      const res = await fetch('/api/household/profile', {
        ...
        body: JSON.stringify({ household_id: householdId, ...draft })
      });
    ...
    <AuthGuard requiredType="household">
      <div className="hh-page">
        <main className="hh-main">
          ...
          <p className="hh-subtitle">Manage your household profile</p>
            ...
            <div className="hh-content-section">
              {!editing ? (
                ...
                <div className="hh-form-grid">
                  <div>
                    <label htmlFor="household-name" className="hh-label">Full Name</label>
                    <input
                      id="household-name"
                      className="hh-input"
                  ...
                  <div>
                    <label htmlFor="household-email" className="hh-label">Email</label>
                    <input
                      id="household-email"
                      className="hh-input"
                      value={draft.email ?? ''}
                  ...
                  <div>
                    <label htmlFor="household-phone" className="hh-label">Phone</label>
                    <input
                      id="household-phone"
                      className="hh-input"
                      value={draft.phone ?? ''}
                  ...
                  <div className="sm:col-span-2">
                    <label htmlFor="household-address" className="hh-label">Address</label>
                    <input
          ...
          {!loading && !profile && householdId && (
            <div className="hh-muted mt-4">No profile found for this household ID.</div>
          )}
        </main>
      </div>
    </AuthGuard>
